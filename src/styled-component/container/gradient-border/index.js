...
const GradiantConatiner = (props) => {
  const { width, height, innerCorner, children } = props;
  return (
    <div
      style={{
        width: width || 400,
    ...
    >
      {innerCorner ? (
        <InnerCornerDiv>{children}</InnerCornerDiv>
      ) : (
        <FlatInnerDiv>{children}</FlatInnerDiv>
      )}
    </div>
  );
};

const InnerCornerDiv = ({ children }) => (
  <>
    <svg
      width="100%"
      height="100%"
...
const FlatInnerDiv = ({ children }) => (
  <>
    <div
      style={{
        width: "100%",
        height: "100%",
        background: `linear-gradient(90deg , #06D9CA 0%, #19B6F0 30%, #6E6CFF 100%)`,
        clipPath:
